...
int Optimizer::OptimizeSim3InterRobot(const vector<cv::Mat>& mapPoints,
                                      ...
                                      KeyFrame *pKF2, vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2, const bool bFixScale)
{
    ...
    // Camera poses
    const cv::Mat R1w = pose.rowRange(0,3).colRange(0,3).clone();
    const cv::Mat t1w = pose.rowRange(0,3).col(3).clone();
//    const cv::Mat R1w = pKF1->GetRotation();
 //   const cv::Mat t1w = pKF1->GetTranslation();
    const cv::Mat R2w = pKF2->GetRotation();
    const cv::Mat t2w = pKF2->GetTranslation();
    ...
    for(int i=0; i<N; i++)
    {
        ...
        // Set edge x1 = S12*X2
        Eigen::Matrix<double,2,1> obs1;
        const cv::KeyPoint &kpUn1 = keypoints[i];
        obs1 << kpUn1.pt.x, kpUn1.pt.y;
        ...
        e12->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(0)));
        e12->setMeasurement(obs1);
        const float &invSigmaSquare1 = mvInvLevelSigma2[kpUn1.octave];
        e12->setInformation(Eigen::Matrix2d::Identity()*invSigmaSquare1);
